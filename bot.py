import discord, json, requests, asyncio, time, pytz
...
async def self(interaction: discord.Interaction, style: discord.app_commands.Choice[int]):   
    try:
        ...
        response = scrapers.githubData(page_data, 0)
...
@tree.command(name="weeklysearch", description="Search for new internship postings in the past 7 hours") 
@app_commands.describe(style="Choose a method for displaying the roles")
@app_commands.choices(style=[
    discord.app_commands.Choice(name="Compact", value=1),
    discord.app_commands.Choice(name="Singular", value=2),
...
async def self(interaction: discord.Interaction, style: discord.app_commands.Choice[int]):   
    try:
        ...
        response = scrapers.githubData(page_data, 1)
...
@tree.command(name="search", description="Search for new internship postings based on a keyword") 
async def self(interaction: discord.Interaction, term: str):
    try:
        page_data = requests.get(links["GITHUB_24"])
...
# TODO: Subscription part is broken
@tasks.loop(hours=24)
